...
mod tests {
    ...
    fn concurrent_insert_if_not_present() {
        const MAX_VALUE: usize = if cfg!(miri) { 128 } else { 512 };
        const NUM_THREADS: usize = if cfg!(miri) { 6 } else { 64 };
    ...
    fn concurrent_insertion() {
        const MAX_VALUE: i32 = if cfg!(miri) { 128 } else { 512 };
        const NUM_THREADS: usize = if cfg!(miri) { 6 } else { 64 };
    ...
    #[test]
    fn remove_if() {
        const NUM_VALUES: i32 = if cfg!(miri) { 128 } else { 512 };
