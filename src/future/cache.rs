...
use crate::{
    ...
    sync::cache::{
        MAX_SYNC_REPEATS, READ_LOG_FLUSH_POINT, READ_LOG_SIZE, WRITE_LOG_FLUSH_POINT,
        WRITE_LOG_HIGH_WATER_MARK, WRITE_LOG_SIZE, WRITE_RETRY_INTERVAL_MICROS,
        WRITE_THROTTLE_MICROS,
    },
};
...
type HouseKeeperArc<K, V, S> = Arc<Housekeeper<Inner<K, V, S>>>;
    ...
    S: BuildHasher + Clone,
{
        ...
        Q: Hash + Eq + ?Sized,
    {
        ...
        if let Some(entry) = self.inner.cache.remove(key) {
            ...
            if Self::schedule_remove_op(&self.write_op_ch, op, hk)
                .await
                .is_err()
            {
    ...
    pub(crate) async fn insert_with_hash(&self, key: K, hash: u64, value: V) {
        self.throttle_write_pace().await;
        let op = self.do_insert_with_hash(key, hash, value);
        let hk = self.housekeeper.as_ref();
        if Self::schedule_insert_op(&self.write_op_ch, op, hk)
            .await
            .is_err()
        {
    ...
    S: BuildHasher + Clone,
{
    ...
    async fn schedule_insert_op(
        ...
        loop {
            Self::apply_reads_writes_if_needed(ch, housekeeper);
            match ch.try_send(op) {
                Ok(()) => break,
    ...
    #[inline]
    fn apply_reads_writes_if_needed(
        ch: &Sender<WriteOp<K, V>>,
        housekeeper: Option<&HouseKeeperArc<K, V, S>>,
    ) {
        let w_len = ch.len();
